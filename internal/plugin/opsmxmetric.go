	...
	for i, item := range metric.Services {
		...
		if isLog {
			...
			if metric.GitOPS && item.LogTemplateVersion == "" {
				shaLog, err := processGitopsTemplates(g, opsmxProfileData, logTemplate, "LOG", item.LogScopeVariables, namespace)
				if err != nil {
					return []service{}, err
				}
		...
		if isMetric {
			...
			if metric.GitOPS && item.MetricTemplateVersion == "" {
				shaMetric, err := processGitopsTemplates(g, opsmxProfileData, metricTemplate, "METRIC", item.MetricScopeVariables, namespace)
				if err != nil {
					return []service{}, err
				}
